...
def normalize_subtype(raw: Optional[str]) -> str:
    ...
    # Collapse whitespace and remove trailing parenthetical details (e.g., "(Attached)")
    value = _SUBTYPE_WS_RE.sub(" ", value)
    value = _SUBTYPE_PARENS_SUFFIX_RE.sub("", value).strip()

    key = value.upper()
    mapped = SUBTYPE_SYNONYMS.get(key)
    if mapped:
        return mapped
