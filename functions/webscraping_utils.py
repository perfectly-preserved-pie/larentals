...
def webscrape_bhhs(url: str, row_index: int, mls_number: str, total_rows: int) -> Tuple[Optional[pd.Timestamp], Optional[str], Optional[str]]:
    ...
    try:
        ...
        if link_tag:
            ...
            logger.info(f"Successfully fetched listing URL for {mls_number} (row {row_index + 1} out of {total_rows}).")
        
        # Fetch MLS photo URL
        photo_tag = soup.find('a', attrs={'class': 'show-listing-details'})
        if photo_tag and 'src' in photo_tag.contents[1]:
            photo = photo_tag.contents[1]['src']
            logger.info(f"Successfully fetched MLS photo for {mls_number} (row {row_index + 1} out of {total_rows}).")
        
        # Fetch list date
        date_tag = soup.find('p', attrs={'class': 'summary-mlsnumber'})
        if date_tag:
            listed_date = pd.Timestamp(date_tag.text.split()[-1])
            logger.info(f"Successfully fetched listed date for {mls_number} (row {row_index + 1} out of {total_rows}).")
