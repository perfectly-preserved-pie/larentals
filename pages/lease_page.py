...
# Define the geojson_filter function using assign()
...
geojson = dlx.dicts_to_geojson(df.to_dict('records'), lon='longitude', lat='latitude')
# Use geopandas to export the GeoJSON to a file
#gpd.GeoDataFrame.from_features(geojson).to_file("assets/datasets/lease.geojson", driver="GeoJSON")
...
lease_geojson = dl.GeoJSON(
    id='lease_geojson',
    url='assets/datasets/lease.geojson',
    #options=dict(filter=geojson_filter),  # Use assign() here
    #hideout={},  # Will be updated via client-side callbacks
    zoomToBounds=True,
)

logger.debug(f"lease_geojson: {lease_geojson}")
