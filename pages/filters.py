...
class LeaseFilters:
    ...
    def sqft_radio_button(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a square footage listed"
            ...
            sqft_choice = self.df['Sqft'].isnull()
            ...
            sqft_choice = self.df.sort_values(by='Sqft')['Sqft'].between(slider_begin, slider_end)
        return (sqft_choice)
    ...
    def yrbuilt_radio_button(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a year built listed"
            # Then we want nulls to be included in the final dataframe
            yrbuilt_choice = self.df['YrBuilt'].isnull()
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            yrbuilt_choice = self.df.sort_values(by='YrBuilt')['YrBuilt'].between(slider_begin, slider_end)
    ...
    def garage_radio_button(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a garage space listed"
            # Then we want nulls to be included in the final dataframe 
            garage_choice = self.df['garage_spaces'].isnull()
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            garage_choice = self.df.sort_values(by='garage_spaces')['garage_spaces'].between(slider_begin, slider_end)
    ...
    def ppsqft_radio_button(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a garage space listed"
            # Then we want nulls to be included in the final dataframe 
            ppsqft_choice = self.df['ppsqft'].isnull()
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            ppsqft_choice = self.df.sort_values(by='ppsqft')['ppsqft'].between(slider_begin, slider_end)
    ...
    def security_deposit_function(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a security deposit listed"
            # Then we want nulls to be included in the final dataframe 
            security_deposit_filter = self.df['DepositSecurity'].isnull() | (self.df.sort_values(by='DepositSecurity')['DepositSecurity'].between(slider_begin, slider_end))
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            security_deposit_filter = self.df.sort_values(by='DepositSecurity')['DepositSecurity'].between(slider_begin, slider_end)
    ...
    def pet_deposit_function(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a security deposit listed"
            # Then we want nulls to be included in the final dataframe 
            pet_deposit_filter = self.df['DepositPets'].isnull() | (self.df.sort_values(by='DepositPets')['DepositPets'].between(slider_begin, slider_end))
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            pet_deposit_filter = self.df.sort_values(by='DepositPets')['DepositPets'].between(slider_begin, slider_end)
    ...
    def key_deposit_function(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a security deposit listed"
            # Then we want nulls to be included in the final dataframe 
            key_deposit_filter = self.df['DepositKey'].isnull() | (self.df.sort_values(by='DepositKey')['DepositKey'].between(slider_begin, slider_end))
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            key_deposit_filter = self.df.sort_values(by='DepositKey')['DepositKey'].between(slider_begin, slider_end)
    ...
    def other_deposit_function(self, boolean, slider_begin, slider_end):
        if boolean == 'True': # If the user says "yes, I want properties without a security deposit listed"
            # Then we want nulls to be included in the final dataframe 
            other_deposit_filter = self.df['DepositOther'].isnull() | (self.df.sort_values(by='DepositOther')['DepositOther'].between(slider_begin, slider_end))
        elif boolean == 'False': # If the user says "No nulls", return the same dataframe as the slider would. The slider (by definition: a range between non-null integers) implies .notnull()
            other_deposit_filter = self.df.sort_values(by='DepositOther')['DepositOther'].between(slider_begin, slider_end)
